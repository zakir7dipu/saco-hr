...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch ?? [],
            'user_avatar' => $userAvatar ?? $profile . '/avatar.png',
        ]);
    }
    ...
    public function getContacts(Request $request)
    {
        ...
        if ($objUser->type != 'super admin') {
            $members = User::where('created_by', '=', $objUser->creatorId())->where('type', '!=', 'super admin')->get();
        }
        $getRecords = null;
        ...
        if ($request->type == 'custom') {
            if (count($usersList) > 0) {
                $contacts = '';
                foreach ($usersList as $user) {
                    $message_count = Chatify::countUnseenMessages($user->id);
                    if (!empty($message_count)) {
                        $contacts_message = Chatify::getContactItem($user);
